...
ルート直下でも `npm run dev` / `npm run build` で同じことができる。
...
| コマンド | 内容 |
|----------|------|
| `python 08-scripts/generate-ogp.py` | 銘柄OGP画像を再生成（銘柄を追加・変更したら必須。写真がある銘柄はボトル写真を合成） |
| `python 08-scripts/whiskies-csv.py export` | 銘柄データをCSVに書き出す（表計算で一括編集用） |
| `python 08-scripts/whiskies-csv.py import` | CSVから銘柄JSONに取り込む |

### 銘柄写真について

49銘柄にWikimedia CommonsのCCライセンス写真（CC0 / CC BY / CC BY-SA）を使っている。

- 画像：`site/public/images/whiskies/*`
...
- 新規に写真を追加するときは、CommonsのカテゴリAPIでライセンスを確認（`article-material-research` スキルの `commons_image.py`）してから
  `site/public/images/whiskies/` に置き、`whisky-photos.json` にクレジットを登録する。CC BY系は帰属表記が必須。

### サイトの構造

| データ | 置き場所 | ページ |
|--------|---------|--------|
| 銘柄 | `site/src/content/whiskies/*.json` | `/whisky/[id]/`、`/whiskies/` |
| 蒸留所 | `site/src/content/distilleries/*.json` | `/distillery/[id]/`、`/distilleries/` |
| 記事 | `site/src/content/blog/*.md` | `/[slug]/`、`/articles/` |
| アフィリURL | `site/src/data/affiliates.ts`（タグは `src/data/site.ts` の `amazonTag`） | — |
...
詳細は `site-requirements.md` §5。

### 記事の書き方

`site/src/content/blog/YYYY-MM-DD-slug.md` を追加する。Frontmatterの主要項目：
...
アフィリエイトURLは記事に直書きせず `site/src/data/affiliates.ts` に集約する。
ウイスキーの数値は `site/src/content/whiskies/` のJSONが唯一の情報源で、
記事・一覧・ランキング・比較・OGPはすべてそこを参照する。

## 最初にやること
1. `04-affiliate/setup-guide.md` を読んでアフィリエイト登録
2. **AmazonアソシエイトのトラッキングIDを `site/src/data/site.ts` の `amazonTag` に設定**
   （この1箇所だけ。全銘柄のAmazonリンクがタグ付きで自動生成される。PA-API不要）
3. 特定の商品ページに固定したい銘柄は `site/src/data/affiliates.ts` に ASIN 付きURLを追加
4. noteアカウント作成
5. Xアカウント作成
6. `06-first-content/week-01/` の記事テンプレを使って最初の5記事を制作
